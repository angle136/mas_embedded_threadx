...
{
    PID_Init_Config_s config = {.MaxOut = 5, .IntegralLimit = 0.01, .DeadBand = 10, .Kp = 0.1, .Ki = 0, .Kd = 0.001, .Improve = 0x01};
    ...
    if (chassis_motors[0] == NULL) return;
    ...
    if (chassis_motors[1] == NULL) return;
    ...
    if (chassis_motors[2] == NULL) return;
    ...
    if (chassis_motors[3] == NULL) return;
...
void chassis_func(Chassis_Ctrl_Cmd_t *chassis_cmd)
{
    ...
    if (chassis_cmd->chassis_mode == chassis_manual)
    {
